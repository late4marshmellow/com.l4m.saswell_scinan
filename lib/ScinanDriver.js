...
const { AUTHORIZATION_URL, LIST_URL, USER_AGENT, CLIENT_ID, REDIRECT_URI, COMPANY_ID, APP_KEY, API_VERSION } = require('./Constants');
...
console.log("Starting getToken function...");
    const response = await fetch(AUTHORIZATION_URL, requestOptions_auth);
    console.log("Made request to AUTHORIZATION_URL, waiting for response...");
  ...
  async getDevicesV1() {
    ...
    console.log("Fetching devices using v1...");
    try {
...
async getDevicesV2() {
    ...
    try {
      ...
      const response = await fetch(LIST_URL, requestOptions_list);
         if (response.ok) {
          const result = await response.json();
...
async getDevices() {
  if (API_VERSION === 'v1') {
      return await this.getDevicesV1();
  } else if (API_VERSION === 'v2') {
      return await this.getDevicesV2();
  } else {
