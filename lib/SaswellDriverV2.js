...
class SaswellDriverV2 extends Homey.Driver {
	...
	async onPair(session) {
		...
		session.setHandler('login', async data => {
			...
			await this.homey.settings.set('md5Password', md5Password);
	...
	async getToken(username, password, md5Password) {
		...
		try {
			...
			this.log('Failed to get token using API v2:', api2Error);
			var errorV2 = api2Error.message;
		}
		...
		try {
			...
			this.log('Failed to get token using API v1:', api1Error);
			var errorV1 = api1Error.message;
		}
		
		throw new Error(`Failed to get token using both API versions. API v2 error: ${errorV2 || 'N/A'}. API v1 error: ${errorV1 || 'N/A'}.`);
	}
	async getTokenV2(username, md5Password) {
		...
		try {
			if (responseData.result_code !== "0") {
                const errorMessage = ERROR_CODES[responseData.result_code] || 'Unknown error occurred.';
                this.log(`Error fetching token: ${responseData.result_code} - ${errorMessage}`);

                // Send notification to the user
                sendNotification(this.homey, `Error fetching token: ${errorMessage}`);
                ...
                switch(responseData.result_code) {
                    ...
                    case "10003":
                        ...
                        this.log(`Token issue: ${errorMessage}`);
                        break;
                    case "30031":
                        // Sign hash issue
                        this.log(`Sign hash invalid: ${errorMessage}`);
                        break;
                    default:
                        // General errors
				...
				token = responseData.resultData.access_token;
				expiresIn = Number(responseData.resultData.expires_in);
		} catch (error) {
