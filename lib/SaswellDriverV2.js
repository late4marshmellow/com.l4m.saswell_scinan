...
//const { APIVersion } = require('../drivers/SASWELL_THERMOSTAT/driver');
    ...
    retrieveCredentials() {
      ...
      async onRepair(session, device) {
        ...
        session.setHandler('login', async data => {
          ...
          await this.getToken(data.username, data.password, deviceId);
      ...
      async getToken(username, password, deviceId) {
        try {
            ...
            const token = await this.getTokenV2(username, password, deviceId);
    ...
    async getTokenV2(username, password, deviceId) {
