    ...
    class SaswellDriverV2 extends Homey.Driver {
        onInit() {
            ...
            this.intervalId = setInterval(() => {
                this.getDeviceDataV2().catch(error => {
                    console.log('Error in getDeviceDataV2:', error);
                });
        ...
        async getDeviceDataV2() {
            if ((this.homey.settings.get('getDeviceDataV2 result_code <> 0')) === true) {
                this.log('getDeviceDataV2 result_code <> 0');
            ...
            //if (apiVersion === "2") {
              ...
              try {
                ...
                if (!response.ok) {
                  ...
                  this.homey.settings.set('last APIv2 result', result)
                  //if result_code is anything else than 0 make device unavailable
                  if (!(result.result_code === "0")) {this.homey.settings.set('getDeviceDataV2 result_code <> 0', true)}
                }
                catch (error) {
                    this.log(error);
      ...
      async onRepair(session, device) {
        ...
        session.setHandler('login', async data => {
          ...
          this.log('Successfully logged in using API v2');
            this.homey.settings.set(`expToken: ${deviceId}`, false);
            this.homey.settings.set(`${deviceId} need repair`, false)
            this.homey.settings.set('getDeviceDataV2 result_code <> 0', false)
            device.setAvailable();
          return true;
        });
