...
const { AUTHORIZATION_URL, LIST_URL, AUTHORIZATION_URL_V2, LIST_URL_V2, USER_AGENT, CLIENT_ID, REDIRECT_URI, COMPANY_ID, APP_KEY, USER_AGENT_V2 } = require('./Constants');
    ...
    class SaswellDriverV2 extends Homey.Driver {
        ...
        async onPair(session) {
          ...
          session.setHandler('start', async data => {
              ...
              const md5Password = createMD5Hash(data.password);
          ...
          session.setHandler('showView', async (view) => {
              if (view === 'loading') {
                  const { username, password } = await this.retrieveCredentials();
    ...
    async retrieveCredentials() {
        ...
        if (this.constructor.APIVersion === "1") {
            ...
            //get the stored md5password and set as password
      ...
      async onRepair(session) {
        ...
        session.setHandler('login', async data => {
          ...
          await this.getToken(data.username, md5Password);
