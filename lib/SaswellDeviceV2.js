...
class SaswellDeviceV2 extends Homey.Device {
  ...
  async getDeviceData() {
    ...
    if (this.homey && this.homey.settings && this.homey.settings.get(`${device.id} need repair`) === true) {
    ...
    if (apiVersion === "2") {
      ...
      try {
        const result = this.homey.settings.get('last APIv2 result');
          //this.log('Device result' + JSON.stringify(result));
          ...
          //this log the s00 output
          this.log('s00 output: ' + JSON.stringify(currentDevice.s00));
    ...
    if (apiVersion === "2") {
      ...
      //if (['away', 'mode', 'onoff'].includes(capability)) {};
        if (result.result_code === "0") {
          ...
          if (capability === 'away'){
            if (value === true)  {
              //store the current target temperature to device settings
              this.setSettings({ awayTargetTemperature: this.getCapabilityValue('target_temperature') });
              this.setCapabilityValue('target_temperature', 5)};
            if (value === false)  {
              //restore the target temperature from device settings 
              this.setCapabilityValue('target_temperature', this.getSettings().awayTargetTemperature)};
          };
          //this.log('set capabilty ' + capability, value)
        }
