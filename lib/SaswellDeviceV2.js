...
class SaswellDeviceV2 extends Homey.Device {
  ...
  async getDeviceData() {
    ...
    if (data && data.id && this.homey && this.homey.settings && this.homey.settings.get(`expToken: ${data.id}`) === true) {
        return;
      }
        ...
        if (!response.ok) {
          ...
          if (!(result.result_code === "0")) { //ERROR_CODES.hasOwnProperty(result.result_code)) {
            ...
            switch (result.result_code) {
              ...
              case '1':
                this.setUnavailable(`Your incorrect password for device ${this.getName()} has been incorrect for 5 times. Please re-authenticate!`)
