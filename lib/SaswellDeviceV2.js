...
const IMEI = this.homey.settings.get('macToImeiMD5');
    ...
    if (apiVersion === "2") {
        ...
        if (response.ok) {
          ...
          if (result.result_code === '10003') {
            ...
            if (this.homey.settings.get(`expToken: ${this.getData().id}`) === false) {
              await sendNotification(this.homey, `Your token has expired. Please re-authenticate!`);
              this.homey.settings.set(`expToken: ${this.getData().id}`, true);
            }
               //throw new Error(`Device: ${this.getName()} Token expired. Please re-authenticate.`);
          }
      ...
      try {
        ...
        if (error.message.includes('getaddrinfo ENOTFOUND api.scinan.com')) {
          if (this.homey.settings.get(`comerr: ${this.getData().id}`) === false) {
          ...
          this.homey.settings.set(`comerr: ${this.getData().id}`, true);
        }}
        throw new Error(error);
      }
