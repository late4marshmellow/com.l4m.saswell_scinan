	...
	async updateCapabilityValues(capability, value) {
		...
		if (apiVersion === "2") {
			...
			try {
				...
				const timestamp = getTimestamp(new Date());
				const response = await fetch(`${CONTROL_URL}?control_data=${control_data}&device_id=${device.id}&format=json&sensor_id=${sensor_id}&sensor_type=1&timestamp=${timestamp}&token=${this.tokenv1}`, {
					method: 'POST',
					headers: {
						'User-Agent': USER_AGENT,
					},
